...
const deconsteFromCartFunction = (id, color) => {
...
const insertTeddy = (id, imgSource, name, color, quantity, price) => {
  ...
  removeButton.addEventListener("click", () =>
    deconsteFromCartFunction(id, color)
  );
...
const validate = () => {
  ...
  const firstNameResult = nameRGEX.test(firstName);
  ...
  const streetRGEX = /^[0-9]{1,3}(?:(?:[,. ]){1}[-a-zA-Zàâäéèêëïîôöùûüç]+)*$/;
  const streetResult = streetRGEX.test(street);
  const city = document.getElementById("city").value;
  const cityRGEX = /^[a-zA-Z ]+$/;
  const cityResult = cityRGEX.test(city);
  const contact = {
    firstName: firstName,
    lastName: lastName,
    adress: street,
...
const confirmation = () => {
  ...
  localStorage.setItem(
    "orderConfirmation",
    JSON.stringify({ orderNumber: orderNumber, orderTotal: orderTotal })
  );
